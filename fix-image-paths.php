...
if (!file_exists('images')) {
    ...
    echo "<p>تم إنشاء مجلد images</p>";
}

// نسخ الصور من img إلى images
echo "<h2>نسخ الصور</h2>";
foreach ($images as $image) {
    $newPath = str_replace('img/', 'images/', $image);
    if (file_exists($image) && !file_exists($newPath)) {
        copy($image, $newPath);
        echo "<p>تم نسخ: " . htmlspecialchars($image) . " إلى " . htmlspecialchars($newPath) . "</p>";
    }
}
?> 